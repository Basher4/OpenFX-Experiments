...
createInstance(OfxImageEffectHandle effect) {
    ...
    gParamHost->paramGetHandle(paramSet, "corner1", &myData->corner1Param, nullptr);
    gParamHost->paramGetHandle(paramSet, "corner2", &myData->corner2Param, nullptr);
    gParamHost->paramGetHandle(paramSet, "colour", &myData->colourParam, nullptr);
...
static OfxStatus render(OfxImageEffectHandle effect,
    ...
    try {
        ...
        if (outputImg == nullptr) throw OfxuNoImageException();


        if (myData->context != eIsGenerator) {
            sourceImg = ofxuGetImage(myData->sourceClip, time, srcRowBytes, srcBitDepth, srcIsAlpha, srcRect, src);
            if (sourceImg == nullptr) throw OfxuNoImageException();

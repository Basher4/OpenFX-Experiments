...
createInstance(OfxImageEffectHandle effect)
{
    ...
    gParamHost->paramGetHandle(paramSet, "scaleComponents", &myData->perComponentScaleParam, nullptr);
    gParamHost->paramGetHandle(paramSet, "scale", &myData->scaleParam, nullptr);
    gParamHost->paramGetHandle(paramSet, "scaleR", &myData->scaleRParam, nullptr);
    gParamHost->paramGetHandle(paramSet, "scaleG", &myData->scaleGParam, nullptr);
    gParamHost->paramGetHandle(paramSet, "scaleB", &myData->scaleBParam, nullptr);
    gParamHost->paramGetHandle(paramSet, "scaleA", &myData->scaleAParam, nullptr);

    // cache away out clip handles
    gEffectHost->clipGetHandle(effect, kOfxImageEffectSimpleSourceClipName, &myData->sourceClip, nullptr);
    gEffectHost->clipGetHandle(effect, kOfxImageEffectOutputClipName, &myData->outputClip, nullptr);

    if (myData->isGeneralEffect) {
        gEffectHost->clipGetHandle(effect, "Mask", &myData->maskClip, nullptr);
    } else
        myData->maskClip = nullptr;
...
template<class PIX, class MASK, int max, int isFloat> class ProcessRGBA : public Processor {
public :
...
template<class PIX, class MASK, int max, int isFloat> class ProcessAlpha : public Processor {
public :
    ...
    OfxPropertySetHandle /*outArgs*/)
{
    ...
    OfxPropertySetHandle sourceImg = nullptr, outputImg = nullptr, maskImg = nullptr;
...
describeInContext(OfxImageEffectHandle effect, OfxPropertySetHandle inArgs)
{
    ...
    // overall scale param
    defineScaleParam(paramSet, "scale", "scale", "scale", "Scales all component in the image", nullptr);
